...
# new_tick_positions = [x for x in np.linspace(min(thetas), max(thetas), int(max(thetas)), endpoint=True) if x % 5 == 0]
...
ax2.set_xlabel('$\\tau_0$')
fig.tight_layout()
fig.savefig(os.path.join(os.getcwd(), 'py-scripts', 'plots', 'results', 'SF-THETA.png'))
...
ax1.grid()
fig.tight_layout()
fig.savefig(os.path.join(os.getcwd(), 'py-scripts', 'plots', 'results', 'SF-THETA-log10.png'))
